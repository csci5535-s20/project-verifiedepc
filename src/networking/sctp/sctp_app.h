...
//            delete fun;
    ...
    #include <list>
    #include <semaphore.h>
    #define MAX_STREAM 64    // needed when defining the SCTP_INITMSG

    class sctp_listener;   // class of threads that listen for connections
    class sctp_callbacks;  // class holding callbacks to ivy

    // A sctp_config maps endpoint ids to IP addresses and ports.
...
class sctp_app {
  public:
    ...
    long long __CARD__a;
    long long __CARD__p;
    long long __CARD__foo__socket;

    sctp_listener *foo__impl__rdr[2];             // the listener task
    sctp_callbacks *foo__impl__cb[2];             // the callbacks to ivy
    hash_space::hash_map<int,sctp_queue *> foo__impl__send_queue[2];   // queues of blocked packets, per socket


    sctp_config *the_sctp_config;  // the current configurations

    // Get the current SCTP configuration. If none, create a default one.

    sctp_config *get_sctp_config() {
        if (!the_sctp_config) 
            the_sctp_config = new sctp_config();
        return the_sctp_config; 
    }

    // Set the current SCTP configuration. This is called by the runtime environment.

    void set_sctp_config(sctp_config *conf) {
        the_sctp_config = conf;
    }

    sctp_app();
    virtual void foo__impl__handle_accept(unsigned prm__X, int s, unsigned other);
    virtual void ext__foo__connected(unsigned self, int s);
    virtual void foo__impl__handle_recv(unsigned prm__X, int s, unsigned x);
    virtual void ext__foo__accept(unsigned self, int s, unsigned other);
    virtual void imp__foo__recv(unsigned self, int s, unsigned p);
    virtual void ext__foo__failed(unsigned self, int s);
    virtual void ext__foo__recv(unsigned self, int s, unsigned p);
    virtual void foo__impl__handle_connected(unsigned prm__X, int s);
    virtual void foo__impl__handle_fail(unsigned prm__X, int s);
    virtual bool ext__foo__send(unsigned self, int s, unsigned p);
    virtual void __init();
