...
#include <cstdint>
#endif
typedef s1_setup_test ivy_class;
...
void s1_setup_test::install_reader(reader *r) {
    #ifdef _WIN32
...
void s1_setup_test::install_thread(reader *r) {
    install_reader(r);
}

void s1_setup_test::install_timer(timer *r) {
    #ifdef _WIN32
...
#ifdef _WIN32
    void s1_setup_test::__lock() { WaitForSingleObject(mutex,INFINITE); }
    void s1_setup_test::__unlock() { ReleaseMutex(mutex); }
#else
    void s1_setup_test::__lock() { pthread_mutex_lock(&mutex); }
    void s1_setup_test::__unlock() { pthread_mutex_unlock(&mutex); }
#endif
...
std::ostream &operator <<(std::ostream &s, const s1_setup_test::message_enum &t);
template <>
s1_setup_test::message_enum _arg<s1_setup_test::message_enum>(std::vector<ivy_value> &args, unsigned idx, long long bound);
template <>
void  __ser<s1_setup_test::message_enum>(ivy_ser &res, const s1_setup_test::message_enum&);
template <>
void  __deser<s1_setup_test::message_enum>(ivy_deser &inp, s1_setup_test::message_enum &res);
int s1_setup_test::___ivy_choose(int rng,const char *name,int id) {
        return 0;
    }
void s1_setup_test::__init(){
    {
        {
...
bool s1_setup_test::ext__ask_and_check_pcode(){
    bool ok;
...
void s1_setup_test::ext__s1_setup__send(unsigned x, message_enum y){
    {
        s1_setup__pcode = x;
        s1_setup__msg = y;
    }
}
unsigned s1_setup_test::ext__ask(){
    unsigned x;
    x = (unsigned)___ivy_choose(0,"fml:x",0);
    x = imp__ask();
    return x;
}
s1_setup_test::message_enum s1_setup_test::ext__s1_setup__recv(){
    s1_setup_test::message_enum y;
    y = (message_enum)___ivy_choose(0,"fml:y",0);
    y = (((s1_setup__msg == message_enum__initial_message) && (s1_setup__pcode == (17 & 255))) ? message_enum__successful_outcome : message_enum__unsuccessful_outcome);
    return y;
}
unsigned s1_setup_test::imp__ask(){
    unsigned x;
    x = (unsigned)___ivy_choose(0,"fml:x",0);
    {
    }
...
void s1_setup_test::__tick(int __timeout){
}
s1_setup_test::s1_setup_test(){
#ifdef _WIN32
...
std::ostream &operator <<(std::ostream &s, const s1_setup_test::message_enum &t){
    if (t == s1_setup_test::message_enum__initial_message) s<<"initial_message";
    if (t == s1_setup_test::message_enum__successful_outcome) s<<"successful_outcome";
    if (t == s1_setup_test::message_enum__unsuccessful_outcome) s<<"unsuccessful_outcome";
    return s;
}
template <>
void  __ser<s1_setup_test::message_enum>(ivy_ser &res, const s1_setup_test::message_enum&t){
    __ser(res,(int)t);
}
...
int ask_ret(long long bound) {
    ...
    class s1_setup_test_repl : public s1_setup_test {
    ...
    s1_setup_test_repl() : s1_setup_test(){}
...
s1_setup_test::message_enum _arg<s1_setup_test::message_enum>(std::vector<ivy_value> &args, unsigned idx, long long bound){
    ...
    if(arg.atom == "initial_message") return s1_setup_test::message_enum__initial_message;
    if(arg.atom == "successful_outcome") return s1_setup_test::message_enum__successful_outcome;
    if(arg.atom == "unsuccessful_outcome") return s1_setup_test::message_enum__unsuccessful_outcome;
...
void __deser<s1_setup_test::message_enum>(ivy_deser &inp, s1_setup_test::message_enum &res){
    int __res;
    __deser(inp,__res);
    res = (s1_setup_test::message_enum)__res;
}
...
class cmd_reader: public stdin_reader {
    int lineno;
public:
    s1_setup_test_repl &ivy;    

    cmd_reader(s1_setup_test_repl &_ivy) : ivy(_ivy) {
        lineno = 1;
        if (isatty(fdes()))
        ...
        try {
            ...
            ivy.__lock();
                ...
                if (action == "s1_setup.send") {
                    ...
                    ivy.ext__s1_setup__send(_arg<unsigned>(args,0,256),_arg<s1_setup_test::message_enum>(args,1,3));
                }
                else
