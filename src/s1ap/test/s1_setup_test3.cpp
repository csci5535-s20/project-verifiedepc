...
#include <cstdint>
#endif
typedef s1_setup_test3 ivy_class;
...
void s1_setup_test3::install_reader(reader *r) {
    #ifdef _WIN32
...
void s1_setup_test3::install_thread(reader *r) {
    install_reader(r);
}

void s1_setup_test3::install_timer(timer *r) {
    #ifdef _WIN32
...
#ifdef _WIN32
    void s1_setup_test3::__lock() { WaitForSingleObject(mutex,INFINITE); }
    void s1_setup_test3::__unlock() { ReleaseMutex(mutex); }
#else
    void s1_setup_test3::__lock() { pthread_mutex_lock(&mutex); }
    void s1_setup_test3::__unlock() { pthread_mutex_unlock(&mutex); }
#endif
...
std::ostream &operator <<(std::ostream &s, const s1_setup_test3::drx_enum &t);
template <>
s1_setup_test3::drx_enum _arg<s1_setup_test3::drx_enum>(std::vector<ivy_value> &args, unsigned idx, long long bound);
template <>
void  __ser<s1_setup_test3::drx_enum>(ivy_ser &res, const s1_setup_test3::drx_enum&);
template <>
void  __deser<s1_setup_test3::drx_enum>(ivy_deser &inp, s1_setup_test3::drx_enum &res);
std::ostream &operator <<(std::ostream &s, const s1_setup_test3::message_enum &t);
template <>
s1_setup_test3::message_enum _arg<s1_setup_test3::message_enum>(std::vector<ivy_value> &args, unsigned idx, long long bound);
template <>
void  __ser<s1_setup_test3::message_enum>(ivy_ser &res, const s1_setup_test3::message_enum&);
template <>
void  __deser<s1_setup_test3::message_enum>(ivy_deser &inp, s1_setup_test3::message_enum &res);
std::ostream &operator <<(std::ostream &s, const s1_setup_test3::supported_ta &t);
template <>
s1_setup_test3::supported_ta _arg<s1_setup_test3::supported_ta>(std::vector<ivy_value> &args, unsigned idx, long long bound);
template <>
void  __ser<s1_setup_test3::supported_ta>(ivy_ser &res, const s1_setup_test3::supported_ta&);
template <>
void  __deser<s1_setup_test3::supported_ta>(ivy_deser &inp, s1_setup_test3::supported_ta &res);
        template <typename T>
        T __array_segment(const T &a, long long lo, long long hi) {
            ...
            return res;
        	    ...
        	    std::ostream &operator <<(std::ostream &s, const s1_setup_test3::supported_ta__arr &a) {
	        s << '[';
	    ...
	    template <>
	    s1_setup_test3::supported_ta__arr _arg<s1_setup_test3::supported_ta__arr>(std::vector<ivy_value> &args, unsigned idx, long long bound) {
	        ivy_value &arg = args[idx];
	        if (arg.atom.size()) 
	            throw out_of_bounds(idx);
	        s1_setup_test3::supported_ta__arr a;
	        a.resize(arg.fields.size());
		for (unsigned i = 0; i < a.size(); i++) {
		    a[i] = _arg<s1_setup_test3::supported_ta>(arg.fields,i,0);
	        }
	        return a;
	    }

	    template <>
	    void __deser<s1_setup_test3::supported_ta__arr>(ivy_deser &inp, s1_setup_test3::supported_ta__arr &res) {
	        inp.open_list();
	    ...
	    template <>
	    void __ser<s1_setup_test3::supported_ta__arr>(ivy_ser &res, const s1_setup_test3::supported_ta__arr &inp) {
	        int sz = inp.size();
	        res.open_list(sz);
	    ...
	    #ifdef Z3PP_H_
	    template <>
            z3::expr __to_solver(gen& g, const z3::expr& z3val, s1_setup_test3::supported_ta__arr& val) {
	    ...
	    template <>
	    void  __from_solver<s1_setup_test3::supported_ta__arr>( gen &g, const  z3::expr &v,s1_setup_test3::supported_ta__arr &res){
	    ...
	    template <>
	    void  __randomize<s1_setup_test3::supported_ta__arr>( gen &g, const  z3::expr &v){
	        unsigned __sz = rand() % 4;
                ...
                g.add_alit(pred);
	        for (unsigned __i = 0; __i < __sz; ++__i)
	            __randomize<s1_setup_test3::supported_ta>(g,g.apply("supported_ta.arr.value",v,g.int_to_z3(g.sort("supported_ta.idx"),__i)));
	    }
	    #endif

	int s1_setup_test3::___ivy_choose(int rng,const char *name,int id) {
        return 0;
    }
unsigned long long s1_setup_test3::supported_ta__arr__begin(const supported_ta__arr& A){
    unsigned long long val;
    val = (unsigned long long)___ivy_choose(0,"ret:val",0);
    val = 0;
    return val;
}
s1_setup_test3::supported_ta s1_setup_test3::supported_ta__arr__value(const supported_ta__arr& a, unsigned long long i){
    s1_setup_test3::supported_ta val;
    val.tac = (unsigned)___ivy_choose(0,"ret:val",0);
    val.plmn_identity2 = (unsigned)___ivy_choose(0,"ret:val",0);
    val =  (0 <= i && i < a.size()) ? a[i] : val ;
    return val;
}
unsigned long long s1_setup_test3::supported_ta__arr__end(const supported_ta__arr& a){
    unsigned long long val;
    val = (unsigned long long)___ivy_choose(0,"ret:val",0);
    val =  a.size() ;
    return val;
}
void s1_setup_test3::ext__s1_setup__send(unsigned x, message_enum y, unsigned a, unsigned b, drx_enum c, supported_ta__arr s){
    {
        s1_setup__pcode = x;
        s1_setup__msg = ((supported_ta__arr__begin(s) < supported_ta__arr__end(s)) ? y : message_enum__unsuccessful_outcome);
        s1_setup__plmn_identity = a;
        s1_setup__global_choice_enb_id = b;
        s1_setup__default_paging_drx = c;
    }
}
s1_setup_test3::supported_ta__arr s1_setup_test3::ext__supported_ta__arr__empty(){
    s1_setup_test3::supported_ta__arr a;
    {
...
bool s1_setup_test3::ext__ask_and_check_pcode(){
    bool ok;
...
s1_setup_test3::message_enum s1_setup_test3::ext__s1_setup__recv(){
    s1_setup_test3::message_enum y;
...
unsigned s1_setup_test3::ext__ask(){
    unsigned x;
    x = (unsigned)___ivy_choose(0,"fml:x",0);
    x = imp__ask();
    return x;
}
unsigned s1_setup_test3::imp__ask(){
    unsigned x;
    x = (unsigned)___ivy_choose(0,"fml:x",0);
    {
    }
...
void s1_setup_test3::__tick(int __timeout){
}
s1_setup_test3::s1_setup_test3(){
#ifdef _WIN32
...
template <>
void  __ser<s1_setup_test3::supported_ta>(ivy_ser &res, const s1_setup_test3::supported_ta&t){
    res.open_struct();
    res.open_field("tac");
...
std::ostream &operator <<(std::ostream &s, const s1_setup_test3::drx_enum &t){
    if (t == s1_setup_test3::drx_enum__0) s<<"0";
    if (t == s1_setup_test3::drx_enum__32) s<<"32";
    if (t == s1_setup_test3::drx_enum__64) s<<"64";
    if (t == s1_setup_test3::drx_enum__128) s<<"128";
    if (t == s1_setup_test3::drx_enum__256) s<<"256";
    return s;
}
template <>
void  __ser<s1_setup_test3::drx_enum>(ivy_ser &res, const s1_setup_test3::drx_enum&t){
    __ser(res,(int)t);
}
std::ostream &operator <<(std::ostream &s, const s1_setup_test3::message_enum &t){
    if (t == s1_setup_test3::message_enum__initial_message) s<<"initial_message";
    if (t == s1_setup_test3::message_enum__successful_outcome) s<<"successful_outcome";
    if (t == s1_setup_test3::message_enum__unsuccessful_outcome) s<<"unsuccessful_outcome";
    return s;
}
template <>
void  __ser<s1_setup_test3::message_enum>(ivy_ser &res, const s1_setup_test3::message_enum&t){
    __ser(res,(int)t);
}
...
int ask_ret(long long bound) {
    ...
    class s1_setup_test3_repl : public s1_setup_test3 {
    ...
    s1_setup_test3_repl() : s1_setup_test3(){}
...
template <>
s1_setup_test3::supported_ta _arg<s1_setup_test3::supported_ta>(std::vector<ivy_value> &args, unsigned idx, long long bound){
    s1_setup_test3::supported_ta res;
    ivy_value &arg = args[idx];
...
s1_setup_test3::drx_enum _arg<s1_setup_test3::drx_enum>(std::vector<ivy_value> &args, unsigned idx, long long bound){
    ...
    if(arg.atom == "0") return s1_setup_test3::drx_enum__0;
    if(arg.atom == "32") return s1_setup_test3::drx_enum__32;
    if(arg.atom == "64") return s1_setup_test3::drx_enum__64;
    if(arg.atom == "128") return s1_setup_test3::drx_enum__128;
    if(arg.atom == "256") return s1_setup_test3::drx_enum__256;
...
void __deser<s1_setup_test3::drx_enum>(ivy_deser &inp, s1_setup_test3::drx_enum &res){
    int __res;
    __deser(inp,__res);
    res = (s1_setup_test3::drx_enum)__res;
}
template <>
s1_setup_test3::message_enum _arg<s1_setup_test3::message_enum>(std::vector<ivy_value> &args, unsigned idx, long long bound){
    ivy_value &arg = args[idx];
    if (arg.atom.size() == 0 || arg.fields.size() != 0) throw out_of_bounds(idx,arg.pos);
    if(arg.atom == "initial_message") return s1_setup_test3::message_enum__initial_message;
    if(arg.atom == "successful_outcome") return s1_setup_test3::message_enum__successful_outcome;
    if(arg.atom == "unsuccessful_outcome") return s1_setup_test3::message_enum__unsuccessful_outcome;
    throw out_of_bounds("bad value: " + arg.atom,arg.pos);
}
template <>
void __deser<s1_setup_test3::message_enum>(ivy_deser &inp, s1_setup_test3::message_enum &res){
    int __res;
    __deser(inp,__res);
    res = (s1_setup_test3::message_enum)__res;
}
...
class cmd_reader: public stdin_reader {
    int lineno;
public:
    s1_setup_test3_repl &ivy;    

    cmd_reader(s1_setup_test3_repl &_ivy) : ivy(_ivy) {
        lineno = 1;
        if (isatty(fdes()))
        ...
        try {
            ...
            ivy.__lock();
                ...
                if (action == "s1_setup.send") {
                    check_arity(args,6,action);
                    ivy.ext__s1_setup__send(_arg<unsigned>(args,0,256),_arg<s1_setup_test3::message_enum>(args,1,3),_arg<unsigned>(args,2,16777216),_arg<unsigned>(args,3,16777216),_arg<s1_setup_test3::drx_enum>(args,4,5),_arg<s1_setup_test3::supported_ta__arr>(args,5,0));
                }
                else
